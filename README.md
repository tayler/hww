...
`hww-gui` takes `--no-rewrite` and `--no-profile` to switch off a per-site table, and
`--show-rewrites` / `--show-profiles` to print one and exit. Image loading (article pictures
on click, and the page favicon) lives behind the `gui` feature; the default build does not
compile it at all.
