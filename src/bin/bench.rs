...
fn main() -> Result<()> {
    ...
    let rows: Vec<Row> = std::fs::read_to_string(&a[1])?
        ...
        .collect();
    let traf: HashMap<String, usize> =
        serde_json::from_str::<Vec<Traf>>(&std::fs::read_to_string(&a[2])?)?
            .into_iter()
            .map(|t| (t.url, t.tr))
            .collect();
    ...
    for r in rows {
        ...
        if !traf.contains_key(&r.url) {
            continue;
        }
        let Ok(bytes) = std::fs::read(format!("{}/{}.raw", a[3], r.key)) else {
            continue;
        };
        let enc =
    ...
    println!(
        "  >=75% of trafilatura's output : {win:3}  ({:.0}%)",
        100.0 * win as f64 / n as f64
    );
    println!(
        "  45-75% (usable, thinner)      : {close:3}  ({:.0}%)",
        100.0 * close as f64 / n as f64
    );
    println!(
        "  <45% (regression)             : {lose:3}  ({:.0}%)",
        100.0 * lose as f64 / n as f64
    );
