...
#[derive(Deserialize)]
struct Row { url: String, key: String, status: Option<u16>, sig: Option<Sig> }
...
fn main() -> Result<()> {
    ...
    for r in rows.iter().filter(|r| r.url.contains(&a[3])) {
        ...
        let Ok(bytes) = std::fs::read(format!("{}/{}.raw", a[2], r.key)) else { continue };
        ...
        println!("== {}\n   visible={} blocks={} text_len={} title={:?}\n   ROOT: {}",
                 r.url, sig.chars, doc.blocks.len(), doc.text_len(), doc.title, html::debug_root(&text));
        if a.len() > 4 {
